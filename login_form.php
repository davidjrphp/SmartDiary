			<form id="login_form1" class="form-signin" method="post">
				<h3 class="form-signin-heading text-white"><i class="icon-lock"></i> Please Login</h3>
				<input type="text" class="input-block-level" id="username" name="username" placeholder="Username" required>
				<input type="password" class="input-block-level" id="password" name="password" placeholder="Password" required>
				<button data-placement="right" title="Click Here to login" id="signin" name="login" class="btn btn-info" type="submit"><i class="icon-signin icon-large"></i> Login</button>
					...
					jQuery("#login_form1").submit(function(e) {
						...
						$.ajax({
							...
							success: function(html) {
								if (html == 'true') {
									$.jGrowl("Welcome to My Smart Diary", {
			...
			<div id="button_form" class="form-signin">
				<p style="color: white">Don't Have SmartDiary Account?</p>
				<hr>
			...
			<script type="text/javascript">
				$(document).ready(function() {
					$('#sign_up_m').tooltip('show');
					$('#sign_up_m').tooltip('hide');
				});
			</script>