...
if (isset($_GET['event'])) {
	...
	$query = mysqli_query($conn, "select * from  contact where id=$contact_id") or die(mysqli_error($conn));
	$row = mysqli_fetch_array($query);

	$number   = $row['number'];
	$name   = $row['name'];
	$mobile   = $row['mobile'];
	$email  = mysqli_real_escape_string($row['email']);
	$work  = $row['work'];
	$address  = mysqli_real_escape_string($row['address']);
	$school  = $row['school'];
	$group  = $row['group'];
				...
				<div class="row-fluid">
					...
					<div id="block_bg" class="block">
						<div class="navbar navbar-inner block-header">
							<?php
							$query = mysqli_query($conn, "select * from contact where user_id = '$id'") or die(mysqli_error($conn));
							...
							$('.delete_event').click(function() {
								...
								$.ajax({
									...
									success: function(html) {
										...
										$('#' + id).modal('hide');
										$.jGrowl("Your Event is Successfully Deleted", {
											header: 'Data Delete'
										});
									}
								});
